//
//  DataStructureUtilities.h
//  tetra
//
//  Created by Alessandro Saccoia on 4/25/17.
//  Copyright © 2017 alsc. All rights reserved.
//

#ifndef DataStructureUtilities_h
#define DataStructureUtilities_h
...
struct Subject {
 public:
  virtual ~Subject() {}
  void subscribe(Observer* aObs) const {
    mObservers.push_back(aObs);
  }
  void unsubscribe(Observer* aObs) const {
    mObservers.erase(std::find(mObservers.begin(), mObservers.end(), aObs));
  }
 protected:
 ...
 private:
  mutable std::list<Observer*> mObservers;
};


}}

#endif /* DataStructureUtilities_h */
